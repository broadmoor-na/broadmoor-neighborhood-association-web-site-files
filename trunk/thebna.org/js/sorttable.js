  ...
  init: function() {
    ...
    forEach(document.getElementsByTagName('table'), function(table) {
      if (table.className.search(/\bsortable\b/) != -1) {
		  ...
		  sorttable.doSort(table.tHead.rows[0].cells[0]);
		  sorttable.doSort(table.tHead.rows[0].cells[0]); // reverse order by time
      }
    });
...
doSort: function(e) {
			...
			// reverse the table, which is quicker
				sorttable.reverse(e.sorttable_tbody);
			...
			if (e.className.search(/\bsorttable_sorted_reverse\b/) != -1) {
		  ...
		  // remove sorttable_sorted classes
			...
			rows = e.sorttable_tbody.rows;
			for (var j=0; j<rows.length; j++) {
				row_array[row_array.length] = [sorttable.getInnerText(rows[j].cells[col]), rows[j]];
			}
			/* If you want a stable sort, uncomment the following line */
			//sorttable.shaker_sort(row_array, e.sorttable_sortfunction);
			/* and comment out this one */
			row_array.sort(e.sorttable_sortfunction);

			tb = e.sorttable_tbody;
